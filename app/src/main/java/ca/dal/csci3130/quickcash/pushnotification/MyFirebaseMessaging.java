...
public class MyFirebaseMessaging extends FirebaseMessagingService {
                ...
                .addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        for (DataSnapshot data : snapshot.getChildren()) {
                            ...
                            if (duration >= jobDuration || salary <= jobSalary || name.equalsIgnoreCase(jobName) || time.equals(jobTime)) {
                                showNotification(notificationBuilder);
                            } else {
                                checkUserLocation(notificationBuilder, jobLatitude, jobLongitude, maxDistance);
                            }
                        }
                    }
                    @Override
     ...
     */
    private void checkUserLocation(NotificationCompat.Builder notificationBuilder, double jobLatitude, double jobLongitude, int maxDistance) {
        ...
        task.addOnSuccessListener(location -> {
            if (location != null) {
                ...
                if (distanceToJob[0] <= maxDistance) {
                    showNotification(notificationBuilder);
                }
            }
        });
